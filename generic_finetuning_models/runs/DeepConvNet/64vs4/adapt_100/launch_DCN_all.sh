...
for ((j=0; j<3; j++))
do
    export scenario=${scenarios[j]}
    for ((i=0; i<17; i++))
    do
        export bucket=${testbuckets[i]}
        export setting=${settings[17*j+i]}
        export restore_file="$YOUR_PATH/fNIRS-mental-workload-classifiers/experiments/generic_models/DeepConvNet/binary/$scenario/$bucket"
        ...
        bash $YOUR_PATH/fNIRS-mental-workload-classifiers/generic_finetuning_models/runs/do_experiment_DeepConvNet_all.slurm
    done
done
