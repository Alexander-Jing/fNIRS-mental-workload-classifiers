    ...
    for test_subject in test_subjects:
        ...
        for lr in lrs:
            for dropout in dropouts:
                ...
                # locate the best model parameter for this subject 
                if restore_file != 'None':
                    subject_summary_filename = os.path.join(restore_file, test_subject, 'hypersearch_summary', 'hypersearch_summary.csv')
                    subject_summary_df = pd.read_csv(subject_summary_filename)
                    subject_best_validation = subject_summary_df.sort_values(by=['validation_accuracy'], ascending=False).iloc[0]
                    subject_experiment_folder = os.path.join(subject_best_validation.experiment_folder, "checkpoint", "best_model.statedict")
                    print('loading checkpoint: {}'.format(subject_experiment_folder))
                    model.load_state_dict(torch.load(subject_experiment_folder, map_location=device))
