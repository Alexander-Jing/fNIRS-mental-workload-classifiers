...
from utils import EarlyStopping, softmax, write_performance_info_FixedTrainValSplit_1
...
def train_classifier(args_dict, train_subjects, val_subjects, test_subjects):
    ...
    for flooding_level in flooding_levels:
        ...
        sampling_points = 150
        #create model
        model = model_to_use(n_class=2, sampling_points=sampling_points, patch_length=30, dim=64, depth=6, heads=8, mlp_dim=256).to(device)
        ...
        for epoch in trange(n_epoch, desc='1-fold cross validation'):
            average_loss_this_epoch = train_one_epoch_Ours_T(model, optimizer, criterion, group_train_loader, device, epoch, flooding_level)
            val_accuracy, _, _, _ = eval_model_OursT(model, group_val_loader, device)
            train_accuracy, _, _ , _ = eval_model_OursT(model, group_train_loader, device)
            ...
            if early_stop(val_accuracy):
                for test_subject in test_subjects:
                    # torch.save(model.state_dict(), os.path.join(test_subjects_dict[test_subject]['result_save_subject_checkpointdir'], 'earlystop_model.statedict'))
                    inference_start_time = time.time()
                    _, _, test_class_labels, test_logits_earlystop = eval_model_OursT(model, test_subjects_dict[test_subject]['sub_test_loader'], device)  # record the results of the last epoch
                    
                    test_prediction_all = 0.2 * softmax(test_logits_earlystop) + 0.8 * softmax(test_subjects_dict[test_subject]['result_save_dict']['bestepoch_test_logits'])
                    class_prediction_all = test_prediction_all.argmax(1)
                    test_accuracy_all = (class_prediction_all == test_class_labels).mean()*100.0
                    print('test combination accuracy this epoch for subject: {} is {}'.format(test_subject, test_accuracy_all))
                    inference_end_time = time.time()
                    inference_time = inference_end_time - inference_start_time
                    test_subjects_dict[test_subject]['result_save_dict']['combination_test_accuracy'] = test_accuracy_all
                break
        ...
        for test_subject in test_subjects:
            ...
            write_performance_info_FixedTrainValSplit(model.state_dict(), test_subjects_dict[test_subject]['result_save_subject_resultanalysisdir'], test_subjects_dict[test_subject]['result_save_dict']['bestepoch_val_accuracy'], test_subjects_dict[test_subject]['result_save_dict']['combination_test_accuracy'])
