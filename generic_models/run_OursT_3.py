...
def train_classifier(args_dict, train_subjects, val_subjects, test_subjects):
    ...
    for flooding_level in flooding_levels:
        ...
        for epoch in trange(n_epoch, desc='1-fold cross validation'):
            average_loss_this_epoch = train_one_epoch_Ours_T_Norm(model, optimizer, criterion, group_train_loader, device, epoch, flooding_level)
            val_accuracy, _, _, _ = eval_model_OursT_Norm(model, group_val_loader, device)
            train_accuracy, _, _ , _ = eval_model_OursT_Norm(model, group_train_loader, device)
            ...
            if is_best:
                ...
                for test_subject in test_subjects:
                    ...
                    test_accuracy, test_class_predictions, test_class_labels, test_logits = eval_model_OursT_Norm(model, test_subjects_dict[test_subject]['sub_test_loader'], device)
