...
def train_classifier(args_dict, train_subjects, val_subjects, test_subjects):
    ...
    #create the group train data and the synthetic data
    group_model_sub_train_feature_list = []
    group_model_sub_train_label_list = []
    group_model_sub_train_feature_synthetic_list = []
    group_model_sub_train_label_synthetic_list = []
    ...
    for subject in train_subjects:
        ...
        group_model_sub_train_feature_synthetic_list.append(sub_feature)
        group_model_sub_train_label_synthetic_list.append(sub_label)  # preparing for the synthetic data
    ...
    group_model_sub_train_feature_synthetic_array = np.array(group_model_sub_train_feature_synthetic_list).astype(np.float32)
    group_model_sub_train_label_synthetic_array = np.array(group_model_sub_train_label_synthetic_list)

    group_model_sub_train_feature_synthetic_array, group_model_sub_train_label_synthetic_array = brain_data.MixUp_expansion(group_model_sub_train_feature_synthetic_array, \
        group_model_sub_train_label_synthetic_array, alpha = 0.75, expand=16)  # generate the synthetic data
    
    _,_,c,t = group_model_sub_train_feature_synthetic_array.shape
    group_model_sub_train_feature_synthetic_array = group_model_sub_train_feature_synthetic_array.reshape(-1,c,t).astype(np.float32)
    group_model_sub_train_label_synthetic_array = group_model_sub_train_label_synthetic_array.reshape(-1).astype(np.int64)
    ...
    #dataset object
    group_train_syn_set = brain_data.brain_dataset(group_model_sub_train_feature_synthetic_array, group_model_sub_train_label_synthetic_array)
    ...
    group_train_syn_loader = torch.utils.data.DataLoader(group_train_syn_set, batch_size=128, shuffle=True)
    ...
    print('data preparing time: {}'.format(data_pre_end-data_pre_start))
    ...
    for flooding_level in flooding_levels:
        ...
        sampling_points = 150
        #create model
        model = model_to_use(n_class=2, sampling_points=sampling_points, patch_length=30, dim=30*8, depth=6, heads=8, mlp_dim=256).to(device)
        ...
        for epoch in trange(n_epoch, desc='1-fold cross validation'):
            ...
            average_loss_this_epoch = train_one_epoch_Ours_T(model, optimizer, criterion, group_train_syn_loader, device, epoch, flooding_level)
            val_accuracy, _, _, _ = eval_model_OursT(model, group_val_loader, device)
            train_accuracy, _, _ , _ = eval_model_OursT(model, group_train_syn_loader, device)
            ...
            if is_best:
                ...
                for test_subject in test_subjects:
                    ...
                    test_accuracy, test_class_predictions, test_class_labels, test_logits = eval_model_OursT(model, test_subjects_dict[test_subject]['sub_test_loader'], device)
